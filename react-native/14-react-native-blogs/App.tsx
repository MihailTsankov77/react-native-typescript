...
import { StyleSheet, SafeAreaView, ScrollView, StatusBar, KeyboardAvoidingView, Platform, FlatList, Animated, Button } from "react-native";
...
import Toaster, { ToastInt } from "./components/CustomToaster";
import CircleDetectRespond from "./components/detectGestures";
import LightBox, { Sample_Images } from "./components/LightBox";
import DragAndDrop from "./DropArea";
...
class App extends Component<{}, AppState> {
  ...
  handleAddFilterTag = (filtersTags: string[])=>{
    this.setState({filterTags: filtersTags});
  ...
  render() {
    ...
    return (
      ...
      //   <Toaster ref={this.ToasterRef} newToasts={this.state.toasterMassage}/>
      // <Button title="ADD" onPress={()=> this.ToasterRef.current?.showMessage([{message: "BLob" + this.state.toasterMassage.length, experationTime: 5000}])}/>
      ...
      //             addMorePosts={this.loadMorePosts}
      //             scrollIndex={this.state.scrollIndex} onFilterTags={this.handleAddFilterTag} tagsFilter={this.state.filterTags}   />
      //             </>);
      ...
      //   </KeyboardAvoidingView>
      // </SafeAreaView>
      // <LightBox height={600} width={300} images={Sample_Images}/>
      // <CircleDetectRespond/>
      <>
        <DragAndDropArea>
          <DropArea />
